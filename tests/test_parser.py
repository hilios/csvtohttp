...
async def test_stream_csv():
    results = []
    async for row in stream_csv("tests/fixtures.csv"):
        results.append(row)
...
async def test_stream_csv_batched():
    results = []
    async for rows in stream_csv("tests/fixtures.csv", batch_size=7):
        results.append(rows)
...
async def test_stream_csv_filter():
    results = []
    async for rows in stream_csv("tests/fixtures.csv", patterns={'gender': '*queer'}):
        results.append(rows)
...
def test_build_request():
    with open('tests/template.hbs', mode='r') as file:
